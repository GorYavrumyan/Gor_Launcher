...
# рекурсивно (файл за файлом, поверх существующих).
#
# "lang" - переводы, обновление им ничем не грозит.
# "core", "shared", "editors", "remote", "addons_sys", "extras" - это код
# самого лаунчера после разбивки GorLauncher.py на модули (см. историю
# рефакторинга) - раньше это были плоские .py файлы в корне и попадали
# под "Правило 3" (одиночный файл, просто перезаписывается), а теперь
# лежат внутри папок - без явного разрешения здесь "Правило 1" тихо
# пропускало бы их целиком, и обновления никогда бы не доходили до кода.
#
# ВАЖНО: "addons" сюда специально НЕ добавлена - это папка АДДОНОВ
# ПОЛЬЗОВАТЕЛЯ (сторонний код, который он сам ставит через Control
# Center), а не код самого лаунчера. Если добавить её в этот список,
# обновление лаунчера будет затирать чужие аддоны поверх - не наше дело
# их трогать.
#
...
UPDATABLE_FOLDERS = {"lang", "core", "shared", "editors", "remote", "addons_sys", "extras"}
...
class UpdateWorker(QThread):
    ...
    Ключевая защита (см. _safe_copy_files):
      1. Любая ПАПКА в корне архива - пропускается (не создаётся, не
         ...
         (lang + модули лаунчера: core/shared/editors/remote/addons_sys/
         extras) - они синхронизируются рекурсивно. "addons" (аддоны
         пользователя) в списке НЕТ намеренно.
    ...
    def _locate_project_source(self, extract_dir):
        ...
        for top in top_entries:
            ...
            if os.path.exists(os.path.join(candidate, "bridge_loader.py")):
                return candidate
        # Резервный вариант: если по какой-то причине bridge_loader.py не
